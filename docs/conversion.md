...
can use. It is split into a **theoretical** part (the geodesy and the equations)
and a **practical** part (the numerical types and the order of operations that
keep the result accurate to the millimetre and free of accumulated rounding
error).
...
For satellite $i$ a receiver measures a *pseudorange* $P_i$ — the apparent
distance from the satellite to the receiver scaled from the signal travel time.
It differs from the true geometric range $\rho_i$ by a sum of physical error
terms ([ESA Navipedia, *Combination of GNSS
Measurements*](https://gssc.esa.int/navipedia/index.php/Combination_of_GNSS_Measurements);
[IS-GPS-200](https://www.gps.gov/technical/icwg/IS-GPS-200N.pdf) §20.3.3.3.3):
...
With WGS-84 constants $\mu = 3.986005\times10^{14}\ \mathrm{m^3/s^2}$ and
$\dot\Omega_e = 7.2921151467\times10^{-5}\ \mathrm{rad/s}$
...
$$
\begin{aligned}
u_k &= \phi_k + C_{us}\sin 2\phi_k + C_{uc}\cos 2\phi_k \\
r_k &= a(1 - e\cos E_k) + C_{rs}\sin 2\phi_k + C_{rc}\cos 2\phi_k \\
i_k &= i_0 + \dot i\,t_k + C_{is}\sin 2\phi_k + C_{ic}\cos 2\phi_k \\
\Omega_k &= \Omega_0 + (\dot\Omega - \dot\Omega_e)\,t_k - \dot\Omega_e\,t_{oe}
\end{aligned}
$$

and finally the Earth-fixed (ECEF) position:

$$
\begin{aligned}
x'_k &= r_k\cos u_k, \quad y'_k = r_k\sin u_k \\
...
rover is an L1 C/A single-frequency receiver
...
16-bit PRC field can hold (§1.8) — but it is identical for every satellite in the
epoch. We estimate it as the **median** of $\text{raw}_i$ across satellites and
subtract it:
...
*every* satellite by a small step. The rover absorbs this into its clock, so
position is unaffected — but it is why the **rate** correction below is computed
on the raw correction, not on the PRC.
:::
...
* **Sentinel avoidance** — the all-ones values ($-32768$ PRC, $-128$ RRC) are
  reserved "satellite problem" flags, so the encoder never emits them (it changes
  scale or clamps to $\pm$max magnitude).
...
Table 20-XIV), with the D29\*/D30\* seed chained across words *and messages* and
the "6-of-8" byte framing. This is {gh}`rtcm3to2p3/parity.py`; §2.9 covers why
this must be done with exact integer arithmetic.
...
(binary64), which has a 52-bit mantissa ≈ 15.95 decimal significant digits. A GPS
...
$\text{raw}_i = \rho_i - P_i - c\,\delta t_{\text{sv}}$ subtracts two ≈
$2.0\text{–}2.6\times10^{7}\ \mathrm{m}$ quantities whose difference is a few
metres. This is
...
nanometres — so the metre-scale difference is preserved to ≈ 9 significant digits.
That headroom is why the code computes each operand at full range and subtracts
directly rather than trying to pre-remove a nominal range.
...
* **Kepler's equation** $E_k = M_k + e\sin E_k$ has no closed form; it is solved
  ...
  linearly and fast; the code iterates to a $10^{-13}\ \mathrm{rad}$ change (well
  below the ULP of the resulting position) with a bounded iteration count so it
  can never spin.
